class ChartController < ApplicationController
  ...
  def show
    ...
    monthly_data = Order.select("DATE_TRUNC('month', created_at) AS month, COALESCE(SUM(CASE WHEN order_status = 6 THEN total_price - refund_amount ELSE total_price END), 0) AS total_sales")
    ...
    chart_data = all_months.map do |month|
      ...
      { "country" => month.strftime("%B"), "value" => (data&.total_sales || 0).to_f }
    end
